...
abstract class REST_Controller extends MY_Controller
{
	...
	protected function _check_login($username = '', $password = NULL)
	{
		...
		$sql = 'SELECT password FROM api_clients WHERE username = ?';
		...
		// If actually NULL (not empty string) then do not check it
		if ($password !== NULL AND $user['password'] != $password)
		{
			return FALSE;
		}
		$this->_client_id =$username;
		$this->_client_password =$user['password'];
		return TRUE;
