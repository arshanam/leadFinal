...
class Fixture {
    ...
    */
    function load($table, $fixt)
    {
        ...
        // This was a necessary fix for Beckett's db schema - since we use multiple
        //  schemas, we had to adjust how the table name was being set
        list($schema, $table) = explode("_", $table, 2);
        $table = $schema . '.' . $table;
    ...
    private function _assign_db()
    {
             ...
             !isset($this->CI->db->database) )
        {
            ...
            $this->CI->db = $this->CI->load->_ci_db;//$this->CI->config->item('db');
        }

        /*foreach($this->CI as $key => $val) {
            if(key_exists('_ci_db', $val)) {
                die(var_dump($key));
            }
        }*/
        //die(var_dump($this->CI->load->_ci_db));
