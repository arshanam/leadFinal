...
class MY_Session extends CI_Session {
    ...
    */
	function _sess_run()
	{	//$this->unset_userdata();exit;
		// Get the current Session Timeout Value
		$currentTimeoutInSecs = 0;
		$currentTimeoutInSecs = ini_get('session.gc_maxlifetime');
		$sessionCookieExpireTime = 6*60*60 + $currentTimeoutInSecs;
		...
		$session_id_ttl = $sessionCookieExpireTime; //$this->object->config->item('sess_expiration');

		if (is_numeric($session_id_ttl))
		{
			if ($session_id_ttl > 0)
			{
				$this->session_id_ttl = $sessionCookieExpireTime; //$this->object->config->item('sess_expiration');
			}
			else
			{
